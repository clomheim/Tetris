...
public final class Board extends Observable
{
  ...
  private void freeze()
  {
    ...
    assignCurrentPiece();
    setChanged();
    notifyObservers(true); //Flag for when a piece freezes.
  }
  ...
  {
    int num_lines = 0; //Counter to keep track of number of lines cleared.
    for (int i = my_blocks.size() - 1; i >= 0; i--)
    {
      ...
      {
        my_blocks.remove(i);
        num_lines++; //increment when a line is removed.
      }
    }
    setChanged();
    notifyObservers(num_lines); //add notification of how many lines have been removed.
  }
